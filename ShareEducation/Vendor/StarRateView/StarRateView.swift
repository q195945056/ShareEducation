...
@IBDesignable public class StarRateView: UIView {
    
    /// 默认整颗星星
    lazy var starType:StarType = .unlimited
    
    /// 是否滑动,default is true
    @IBInspectable var isPanEnable: Bool = false{
        didSet{
            if isPanEnable {
                /// 滑动手势
    ...
    @IBInspectable lazy var starSpace:CGFloat = 5.0
    
    /// 当前的星星数量,default is 0
    @IBInspectable var currentStarCount:CGFloat = 0 {
        didSet {
            showStarRate()
        }
    }
    ...
    /// 星星总数量,default is 5
    @IBInspectable lazy var totalStarCount:CGFloat = 5
